...
rng('shuffle'); %random number generation using seed at current time

addpath('./my_utils'); %adds search path to matlab session
...
run_config.use_gpu=false; %specifically this
...
% or specify a model_name for training, if the cached snapshot files existed, it will resume training
 model_name='refinenet_res101_cityscapes';
...
ds_name='SkyFinder'; %dataset name
gen_ds_info_fn=@my_gen_ds_info_skyfinder;%generate information on dataset
...
%writing to file to track progress
