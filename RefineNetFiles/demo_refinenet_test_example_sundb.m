...
function demo_refinenet_test_example_cityscapes()
    wd = '../datasets/SUNdb/SkyImages';
    img_dir = dir(wd);
    img_dir = img_dir((~ismember({img_dir.name},{'.','..'})));
    for i = 1:length(img_dir) %a-z
        subfolders = dir(fullfile(wd,img_dir(i).name));
        subfolders=subfolders(~ismember({subfolders.name},{'.','..'}));

        for j = 1:length(subfolders)%abbey airport
            subsub = dir(fullfile(wd,img_dir(i).name,subfolders(j).name));
            subsub=subsub(~ismember({subsub.name},{'.','..'}));

            if (strfind(subsub(1).name, '.jpg'))
                    rng('shuffle');
                    ...
                    run_config.trained_model_path='../model_trained/I1/epoch_10/net-config-epoch-10.mat';
                    ds_config.class_info=gen_class_info_skyfinder();
                ...
                for k = 1:length(subsub)%airport terminal airport gate
                        rng('shuffle');
                        ...
                        % result dir:
                        %result_name=['result_' datestr(now, 'YYYYmmDDHHMMSS') '_evaonly_custom_data'];
                        result_name=subsub(k).name;
                        result_dir=fullfile('../cache_data', 'test_examples_cityscapes', result_name);


                        % the folder that contains testing images:
                        ds_config.img_data_dir=fullfile(wd,img_dir(i).name,subfolders(j).name,subsub(k).name);
